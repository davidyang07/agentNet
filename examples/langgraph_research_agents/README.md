...
to `sentinel_agent`. This is the external application AgentNet's importer
...
so AgentNet's backend has no LangGraph runtime dependency.

## Regenerating `topology.json`

`topology.json` (committed) is AgentNet's import format: agent ids, communication edges, tool
...
## What AgentNet does with it

`backend/scripts/run_external_import_demo.py` loads the committed `topology.json` (no LangGraph
install required for this step — it's a plain JSON file), builds an AgentNet `WorldState` from the
...
remediation recommendations — exactly like any AgentNet-native experiment.
